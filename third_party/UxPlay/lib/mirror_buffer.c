...
mirror_buffer_init_aes(mirror_buffer_t *mirror_buffer, const uint64_t *streamConnectionID)
{
    ...
    /* destroy any previous AES context and reset counter state, otherwise a
     * second mirror SETUP (e.g. after returning from HLS playback) would leak
     * the old context and start decryption with a stale CTR offset, producing
     * "decryption of video packet failed" for every incoming packet. */
