...
*hls_get_current_video(raop_t *raop) {
    ...
    if (!raop->airplay_video[raop->current_video]) {
        logger_log(raop->logger, LOGGER_ERR,
                   "hls_get_current_video: current_video index %d points to a NULL playlist",
                   raop->current_video);
    ...
    */
    ...
    if (!airplay_video) {
        logger_log(raop->logger, LOGGER_WARNING,
                   "POST /setProperty?%s ignored: no current HLS playlist (likely after EOS)", property);
        goto post_error;
    }
                           ...
                           char **response_data, int *response_datalen)
{
    ...
    printf("__AIRMIRROR_HLS__ event=alive\n");
    fflush(stdout);
...
http_handler_action(raop_conn_t *conn, http_request_t *request, http_response_t *response,
    ...
    if (!airplay_video) {
        logger_log(raop->logger, LOGGER_WARNING,
                   "POST /action ignored: no current HLS playlist (likely after EOS)");
...
http_handler_hls(raop_conn_t *conn,  http_request_t *request, http_response_t *response,
    ...
    if (!airplay_video) {
        logger_log(raop->logger, LOGGER_WARNING,
                   "GET %s ignored: no current HLS playlist (likely after EOS)", url);
        http_response_init(response, "HTTP/1.1", 404, "Not Found");
        return;
    }
    if (!strcmp(url, "/master.m3u8")){
        ...
        if (media_playlist) {
            ...
            *response_datalen = strlen(data);
            ...
            printf("__AIRMIRROR_HLS__ event=duration value=%f\n", duration);
            fflush(stdout);
        } else {
