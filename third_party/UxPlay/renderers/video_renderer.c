...
#define WINDOWED_SCREEN_FRACTION 0.80
#define MIN_WINDOWED_WIDTH 320
#define MIN_WINDOWED_HEIGHT 240
#ifdef X_DISPLAY_FIX
...
struct video_renderer_s {
    ...
    gboolean windowed_state_saved;
    gboolean win32_initial_fullscreen;
    gboolean win32_initial_action_applied;
    gboolean win32_is_fullscreen;
    guint win32_esc_poll_id;
#endif
#ifdef  X_DISPLAY_FIX
    bool use_x11;
...
#ifdef _WIN32
static const char uxplay_renderer_prop[] = "UxPlayRenderer";
static const char uxplay_prev_wndproc_prop[] = "UxPlayPrevWndProc";

typedef struct win32_window_search_s {
    DWORD process_id;
    HWND hwnd;
    LONG best_area;
} win32_window_search_t;

static void win32_set_fullscreen(video_renderer_t *renderer, gboolean fullscreen_state);
static gboolean win32_esc_poll_cb(gpointer user_data);

static BOOL CALLBACK win32_enum_windows_callback(HWND hwnd, LPARAM lParam) {
    win32_window_search_t *search = (win32_window_search_t *) lParam;
    DWORD window_process_id = 0;
    RECT rect;

    GetWindowThreadProcessId(hwnd, &window_process_id);
    if (window_process_id != search->process_id || !IsWindowVisible(hwnd) || GetAncestor(hwnd, GA_ROOT) != hwnd) {
        return TRUE;
    }
...
static HWND win32_find_renderer_window(void) {
    win32_window_search_t search;
    search.process_id = GetCurrentProcessId();
    search.hwnd = NULL;
    search.best_area = 0;
    EnumWindows(win32_enum_windows_callback, (LPARAM) &search);
    return search.hwnd;
}

static void win32_raise_window(HWND hwnd) {
...
static RECT win32_calculate_windowed_rect(HWND hwnd) {
    ...
    if (GetMonitorInfoA(MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST), &monitor_info)) {
        work = monitor_info.rcWork;
    } else {
        SystemParametersInfoA(SPI_GETWORKAREA, 0, &work, 0);
    }

    if (!hls_video && width > 0 && height > 0) {
        aspect = ((double) width) / ((double) height);
    } else if (GetWindowRect(hwnd, &current)) {
        int current_width = current.right - current.left;
        int current_height = current.bottom - current.top;
        if (current_width > 0 && current_height > 0) {
            aspect = ((double) current_width) / ((double) current_height);
        }
    }
    ...
    max_width = (int) (work_width * WINDOWED_SCREEN_FRACTION);
    max_height = (int) (work_height * WINDOWED_SCREEN_FRACTION);
    ...
    if (target_width < MIN_WINDOWED_WIDTH && max_width >= MIN_WINDOWED_WIDTH) {
        target_width = MIN_WINDOWED_WIDTH;
        target_height = (int) (target_width / aspect);
    }

    if (target_height < MIN_WINDOWED_HEIGHT && max_height >= MIN_WINDOWED_HEIGHT) {
        target_height = MIN_WINDOWED_HEIGHT;
        target_width = (int) (target_height * aspect);
    }
    ...
    result.left = work.left + (work_width - target_width) / 2;
    result.top = work.top + (work_height - target_height) / 2;
    result.right = result.left + target_width;
    result.bottom = result.top + target_height;
    return result;
}

static void win32_save_current_windowed_state(video_renderer_t *renderer) {
    if (!renderer || !renderer->hwnd || !IsWindow(renderer->hwnd)) {
        return;
    }

    renderer->windowed_style = GetWindowLongPtrA(renderer->hwnd, GWL_STYLE);
    renderer->windowed_ex_style = GetWindowLongPtrA(renderer->hwnd, GWL_EXSTYLE);
    GetWindowRect(renderer->hwnd, &renderer->windowed_rect);
    renderer->windowed_state_saved = TRUE;
}

static void win32_save_preferred_windowed_state(video_renderer_t *renderer) {
    if (!renderer || !renderer->hwnd || !IsWindow(renderer->hwnd)) {
        return;
    }
    ...
    renderer->windowed_style = GetWindowLongPtrA(renderer->hwnd, GWL_STYLE);
    renderer->windowed_ex_style = GetWindowLongPtrA(renderer->hwnd, GWL_EXSTYLE);
    renderer->windowed_rect = win32_calculate_windowed_rect(renderer->hwnd);
    renderer->windowed_state_saved = TRUE;
}

static void win32_apply_windowed_bounds(video_renderer_t *renderer) {
    RECT rect;
    ...
    rect = win32_calculate_windowed_rect(renderer->hwnd);
    SetWindowPos(renderer->hwnd, HWND_TOP, rect.left, rect.top,
                 rect.right - rect.left, rect.bottom - rect.top,
                 SWP_NOOWNERZORDER | SWP_FRAMECHANGED | SWP_SHOWWINDOW);
    win32_save_current_windowed_state(renderer);
    renderer->win32_is_fullscreen = FALSE;
    win32_raise_window(renderer->hwnd);
}
...
static void win32_set_fullscreen(video_renderer_t *renderer, gboolean fullscreen_state) {
    ...
    if (fullscreen_state) {
        if (!renderer->win32_is_fullscreen) {
            if (renderer->win32_initial_action_applied) {
                win32_save_current_windowed_state(renderer);
            } else if (!renderer->windowed_state_saved) {
                win32_save_preferred_windowed_state(renderer);
            }
        }

        monitor_info.cbSize = sizeof(MONITORINFO);
        if (GetMonitorInfoA(MonitorFromWindow(renderer->hwnd, MONITOR_DEFAULTTONEAREST), &monitor_info)) {
            rect = monitor_info.rcMonitor;
        } else {
            SystemParametersInfoA(SPI_GETWORKAREA, 0, &rect, 0);
        }

        style = GetWindowLongPtrA(renderer->hwnd, GWL_STYLE);
        ex_style = GetWindowLongPtrA(renderer->hwnd, GWL_EXSTYLE);
        ...
        SetWindowLongPtrA(renderer->hwnd, GWL_STYLE, style);
        SetWindowLongPtrA(renderer->hwnd, GWL_EXSTYLE, ex_style);
        SetWindowPos(renderer->hwnd, HWND_TOP, rect.left, rect.top,
                     rect.right - rect.left, rect.bottom - rect.top,
                     SWP_NOOWNERZORDER | SWP_FRAMECHANGED | SWP_SHOWWINDOW);
        renderer->win32_is_fullscreen = TRUE;
        if (renderer->win32_esc_poll_id == 0) {
            renderer->win32_esc_poll_id = g_timeout_add(50, win32_esc_poll_cb, renderer);
        }
    } else {
        if (!renderer->windowed_state_saved) {
            win32_save_preferred_windowed_state(renderer);
        }

        SetWindowLongPtrA(renderer->hwnd, GWL_STYLE, renderer->windowed_style);
        SetWindowLongPtrA(renderer->hwnd, GWL_EXSTYLE, renderer->windowed_ex_style);
        rect = renderer->windowed_rect;
        SetWindowPos(renderer->hwnd, HWND_TOP, rect.left, rect.top,
                     rect.right - rect.left, rect.bottom - rect.top,
                     SWP_NOOWNERZORDER | SWP_FRAMECHANGED | SWP_SHOWWINDOW);
        renderer->win32_is_fullscreen = FALSE;
...
static gboolean win32_esc_poll_cb(gpointer user_data) {
    video_renderer_t *renderer = (video_renderer_t *) user_data;
    if (!renderer || !renderer->hwnd || !IsWindow(renderer->hwnd) || !renderer->win32_is_fullscreen) {
        if (renderer) renderer->win32_esc_poll_id = 0;
        return G_SOURCE_REMOVE;
    }
    SHORT esc = GetAsyncKeyState(VK_ESCAPE);
    if (esc & 0x8000) {
        HWND fg = GetForegroundWindow();
        if (fg == renderer->hwnd || IsChild(renderer->hwnd, fg)) {
            win32_set_fullscreen(renderer, FALSE);
            renderer->win32_esc_poll_id = 0;
            return G_SOURCE_REMOVE;
        }
    }
...
static LRESULT CALLBACK win32_renderer_wndproc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    video_renderer_t *renderer = (video_renderer_t *) GetPropA(hwnd, uxplay_renderer_prop);
    WNDPROC previous = (WNDPROC) GetPropA(hwnd, uxplay_prev_wndproc_prop);

    if (renderer && (message == WM_KEYDOWN || message == WM_SYSKEYDOWN)) {
        if (wParam == VK_F11 || (wParam == VK_RETURN && (GetKeyState(VK_MENU) & 0x8000))) {
            win32_set_fullscreen(renderer, !renderer->win32_is_fullscreen);
            return 0;
        }
        if (wParam == VK_ESCAPE && renderer->win32_is_fullscreen) {
            win32_set_fullscreen(renderer, FALSE);
            return 0;
        }
    }
    ...
    if (previous) {
        return CallWindowProcA(previous, hwnd, message, wParam, lParam);
    }

    return DefWindowProcA(hwnd, message, wParam, lParam);
}

static void win32_subclass_renderer_window(video_renderer_t *renderer) {
    if (!renderer || !renderer->hwnd || !IsWindow(renderer->hwnd)) {
        return;
    }

    if (GetPropA(renderer->hwnd, uxplay_prev_wndproc_prop)) {
        return;
    }

    SetPropA(renderer->hwnd, uxplay_renderer_prop, (HANDLE) renderer);
    renderer->original_wndproc = (WNDPROC) SetWindowLongPtrA(renderer->hwnd, GWLP_WNDPROC,
                                                             (LONG_PTR) win32_renderer_wndproc);
    SetPropA(renderer->hwnd, uxplay_prev_wndproc_prop, (HANDLE) renderer->original_wndproc);
}

static void win32_restore_renderer_window(video_renderer_t *renderer) {
    if (!renderer || !renderer->hwnd || !IsWindow(renderer->hwnd)) {
        return;
    }

    if (renderer->original_wndproc) {
        SetWindowLongPtrA(renderer->hwnd, GWLP_WNDPROC, (LONG_PTR) renderer->original_wndproc);
    }

    RemovePropA(renderer->hwnd, uxplay_renderer_prop);
    RemovePropA(renderer->hwnd, uxplay_prev_wndproc_prop);
}

static gboolean win32_manage_renderer_window(video_renderer_t *renderer) {
    if (!renderer) {
        return FALSE;
    }

    if (!renderer->hwnd || !IsWindow(renderer->hwnd)) {
        renderer->hwnd = win32_find_renderer_window();
        if (!renderer->hwnd) {
            return FALSE;
        }
        win32_subclass_renderer_window(renderer);
        logger_log(logger, LOGGER_INFO, "Windows renderer: F11 or Alt-Enter toggles fullscreen; Esc exits fullscreen");
    }

    if (!renderer->win32_initial_action_applied) {
        if (renderer->win32_initial_fullscreen) {
            win32_save_preferred_windowed_state(renderer);
            win32_set_fullscreen(renderer, TRUE);
        } else {
            win32_apply_windowed_bounds(renderer);
        }
        renderer->win32_initial_action_applied = TRUE;
    }
...
#ifdef _WIN32
        renderer_type[i]->hwnd = NULL;
        renderer_type[i]->original_wndproc = NULL;
        renderer_type[i]->windowed_state_saved = FALSE;
        renderer_type[i]->win32_initial_fullscreen = initial_fullscreen ? TRUE : FALSE;
        renderer_type[i]->win32_initial_action_applied = FALSE;
        renderer_type[i]->win32_is_fullscreen = FALSE;
#endif
        if (hls_video) {
            ...
            if (!auto_videosink) { 
                ...
                if (!strcmp(videosink, "fakesink")) {
                    GstElement *playbin_audiosink = gst_element_factory_make("fakesink", "hls-audio-fakesink");
                    if (!playbin_audiosink) {
                        logger_log(logger, LOGGER_ERR, "video_renderer_init: failed to create playbin_audiosink");
                    } else {
                        logger_log(logger, LOGGER_DEBUG, "video_renderer_init: create playbin_audiosink at %p", playbin_audiosink);
                        g_object_set(G_OBJECT (renderer_type[i]->pipeline), "audio-sink", playbin_audiosink, NULL);
                    }
                }
            }
            gint flags = 0;
...
#ifdef _WIN32
    win32_manage_renderer_window(renderer);
#endif
}
...
#ifdef _WIN32
    if (renderer && !win32_manage_renderer_window(renderer)) {
        return true;
    }
#endif
#ifdef X_DISPLAY_FIX
...
#ifdef _WIN32
        win32_restore_renderer_window(renderer);
#endif
...
int video_renderer_choose_codec (bool video_is_jpeg, bool video_is_h265) {
    ...
    if (hls_video) {
        logger_log(logger, LOGGER_WARNING, "video_renderer_choose_codec ignored while HLS renderer is shutting down");
        return 0;
    }
    if (video_is_jpeg) {
...
/* AirMirror stubs: feature not implemented for in-app HLS playback. */
void video_renderer_set_parent_hwnd(unsigned long long parent) { (void) parent; }
int  video_renderer_get_audio_track_count(void) { return 0; }
int  video_renderer_get_current_audio_track(void) { return -1; }
int  video_renderer_get_audio_track_label(int index, char *out, int out_len) { (void) index; if (out && out_len > 0) out[0] = 0; return 0; }
void video_renderer_set_audio_track(int index) { (void) index; }
